...
for line in decoded:
    ...
    print(url_gff3)
    dico[index] = [scientific_name,taxid,assembly_name,assembly_accession,division,url_fasta_cds,url_fasta_pep,url_fasta_genome,url_gff3] 
    accessions.append(assembly_accession)
    index +=1

sum = pd.DataFrame(dico).T
sum.columns = ['Species Name','Taxid','Assembly Name','Assembly Accession',"Division","URL fasta cds","URL fasta proteins","URL fasta genome","URL gff3 genome"]
print(sum)
