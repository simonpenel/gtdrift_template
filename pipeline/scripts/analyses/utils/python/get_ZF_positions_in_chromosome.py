...
with open(args.input, 'r') as reader:
    ...
    for line in lines:
        splitline = line.split(";")
        prot_name = splitline[0]
        print("debug "+prot_name)
        if prot_name in dico_prot :
            chromo_info = dico_prot[prot_name]
        else :
            print("Warning: No chromosome information for " + prot_name)
            chromo_info = ["NA", "NA", [0,0], 0]
        chromo = chromo_info[0]
        strand = chromo_info[1]
        positions = chromo_info[2]
        pseudo = chromo_info[3]
        new_row = {"SeqID": prot_name, "Chromosome": chromo, "Chr Start": positions[0],"Chr End":  positions[1], "Strand" : strand, "Pseudo" : pseudo}
        df = df._append(new_row, ignore_index=True)
        

df_summary = pd.read_csv(args.input, sep=';', header=0,names=['SeqID','ZF start','ZF end'])
print(df_summary)
#df_summary = df_summary.drop(["Unnamed: 0"],axis=1) 

if len(df.index) > 0 :
    newdf = df.join(df_summary.set_index('SeqID'),on='SeqID', how="outer",lsuffix='_caller', rsuffix='_other')
else:
    newdf = df_summary
    newdf['Chromosome'] = []
    newdf['Chr Start'] = []
