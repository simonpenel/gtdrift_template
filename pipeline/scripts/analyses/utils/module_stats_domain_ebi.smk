...
rule hmm_search:
    ...
    input:
        ...
        # the proteome
        protein=pathGTDriftData + "genome_assembly_ebi/{accession}/annotation/protein.faa",
    output:
        # table of of per-sequence hits
        table=pathGTDriftData
        + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "hmm_search/tbl/{domain}",
        # table of per-domain hits
        domains=pathGTDriftData
        + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "hmm_search/domtbl/{domain}_domains",
    shell:
...
rule formating_hmm_sequence_hit:
    ...
    output:
        ...
        pathGTDriftData
        + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "hmm_search/tbl/{domain}_tabulated",
    script:
...
rule formating_hmm_domain_hit:
    ...
    output:
        ...
        + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "hmm_search/domtbl/{domain}_domains_summary",
    script:
...
rule summarize_hmm_results:
    ...
    input:
        ...
        domain_per_domain_summary=pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "hmm_search/domtbl/{domain}_domains_summary",        
    output:
        # domain protein statistics for each assembly.
        pathGTDriftData
        + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS +"summary_hmmsearch_{accession}_{domain}.csv",
    params:    
