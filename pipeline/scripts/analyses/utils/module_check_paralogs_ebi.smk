...
rule generate_domain_candidates_IDs:
    ...
    output:
        candidate_list = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "candidates_simple_{domain}.txt"    
    shell:
        """
...
rule run_seqkit_extract:
    input:
        candidate_list = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "candidates_simple_{domain}.txt",
        multifasta = pathGTDriftData + "genome_assembly_ebi/{accession}/annotation/protein.faa"
    output:
        fasta_output = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "candidates_1_{domain}.fasta"
    shell:
        """
...
rule hmmscan:
    input:
        ...
        fasta = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "candidates_1_{domain}.fasta",
        candidate_table = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "summary_hmmsearch_{accession}_{domain}.csv"
    output:
        hmm = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "results_HMM_score_ratio_{domain}.csv",
        table = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "table_score_ratios_HMM_{domain}.csv",
...
rule curate_prdm9_candidates_IDs:
    input:
         candidates = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "candidates_{domain}.csv",
    output:
        candidate_list = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "candidates_ID_{domain}.txt"
    script:
...
rule run_seqkit_extraction_curated:
    input:
        ...
        multifasta = pathGTDriftData + "genome_assembly_ebi/{accession}/annotation/protein.faa"
    output:
        fasta_output = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "candidates_{domain}.fasta"
    shell:
        """
        seqkit grep -f {input.candidate_list} {input.multifasta} -o {output.fasta_output}
        """
...
rule generate_candidates_wad_IDs:
    input:
        candidates = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "selected_candidates.csv"
    output:
        candidate_list = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "selected_candidates.txt"   
    script:
        "../utils/python/extract_domain_candidates_ID.py"
...
rule run_seqkit_extraction_wad_fasta:
    input:
        ...
        multifasta = pathGTDriftData + "genome_assembly_ebi/{accession}/annotation/protein.faa"
    output:
        fasta_output = pathGTDriftData + "genome_assembly_ebi/{accession}/analyses/" + GENOME_RESULTS + "selected_candidates.fasta"
    shell:
        """
        seqkit grep -f {input.candidate_list} {input.multifasta} -o {output.fasta_output}
        """
